...
`dev/20260801_145839_complete_development_plan_lexml_non_statutory_parser/20260910_000405_cycle_8e_report.md`.
**Executed 2026-09-10.** Amendment **A-L.9** was added by the cycle, from
measurement taken before implementation. Two figures in this section were
stale and are corrected in place: the golden count is **135**, not 125, and
the measured per-type resolution rate is in the report.
...
| A-L.5 | §2.11, §9.2 (invariant #12) | **The linker is probed, never assumed — the third external thing this repository works without.** `refs/probe.py` follows `validate/schema.py:359` `probe_capabilities` exactly: it never raises, and a missing binary answers `available=False` with a diagnostic saying *why*. `tests/conftest.py` gains `requires_linker` beside `requires_nested` (`:98`), carrying the probe's own diagnostic as the skip reason. Nothing branches on a linker *version*; only on what a probe of the binary actually present reported. **The binary is available in this environment at `/usr/local/bin/linkertool`** — a symlink onto the Haskell build tree at `../lexml-linker/dist-newstyle/build/x86_64-linux/ghc-9.6.7/lexml-parser-1.4.10/x/linkertool/build/linkertool/linkertool` — and that path is also upstream's own default for the `lexml.linkertool` property (`Linker.scala:25`), so `probe.py` searches it first, then `PATH`. Its presence here is what lets the cycle's *binary present* exit criterion actually run; it changes nothing about the default, which stays `none`, and nothing about the bare-checkout criterion, which is still exercised by pointing the probe at a path that does not exist. *Decided with the user (2026-09-08)* |
| A-L.6 | §9.3, §9.4 | **Fixtures are the seam, and the existing 125 goldens must not move.** `refs/cache.py` reuses `referee/cache.py`'s shape — one JSON file per key, `read_only` for fixture directories — keyed by `(context_urn, paragraph text)`. Existing golden kinds render with `NullLinker`, so **all 135 stay byte-identical** (the count was stale in this row: 9 kinds × 15 samples, plus 3 annex/extra files); two new kinds, `generico-linked` and `norma-linked`, join `scripts/regen_goldens.py`'s `KINDS` (`:186`) and are generated against the fixture-backed linker, so they compare with **no binary present**. Refreshing fixtures is an explicit documented command, `scripts/record_linker_fixtures.py`, never automatic — the §9.3 rule, so a linker upgrade shows up as a reviewed diff |
...
| A-L.9 | §18, A-L.8 | **The context URN is always `urn:lex:br:federal:lei:2000-01-01;1` — the fallback is the only usable value.** *Added by Cycle 8e, from measurement.* A-L.8 required the context to be measured against `parseURNLexML` before use, and named the reference parser's default as the fallback. Measured, component by component, the result is sharper than expected: a non-statutory **type** token (`parecer`, `ato.declaratorio`, `portaria`) links normally, and so does A-2.3's `0000` year **sentinel** — the two things A-L.8 predicted would break. What breaks it is the **authority**: any value other than a recognised one (`ministerio.fazenda`, `superior.tribunal.justica`, or any `;`-joined pair) makes the linker return the fragment **unchanged**, resolving nothing, with no error. All 15 samples carry such an authority, so a document's own URN would resolve zero references corpus-wide while looking exactly like a document that cites nothing — the worst shape a failure can take. `find_refs` keeps `context_urn` a parameter so a linker that later learns these authorities needs no new API; every caller in the package passes the constant. Pinned by `test_a_non_federal_authority_context_resolves_nothing` so a linker that starts accepting them is a *finding*, not a silent change |

### Cycle 8e — External reference URNs (deliverables)

- `src/lexml_nonstat/refs/` — `protocol.py`, `null.py`, `linkertool.py`,
  `cache.py`, `probe.py`; `probe.py` looks for the binary at
  `/usr/local/bin/linkertool` first, then on `PATH` (A-L.1, A-L.5)
...
- **all 135 existing goldens byte-identical** — the cycle's first exit criterion
  (the plan said 125; the true figure across the 9 pre-existing kinds is 135)
...
- **binary present** — `scripts/record_linker_fixtures.py` reproduces every
  ...
  This criterion is runnable in this environment: `linkertool` is installed at
  `/usr/local/bin/linkertool` (A-L.5)
